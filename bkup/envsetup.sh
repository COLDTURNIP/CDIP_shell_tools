...
- mgrep:   Greps on all local make files.
- sgrep:   Greps on all local source files (c, h, cpp, S, java, xml, sh, mk).
- resgrep: Greps on all local res/*.xml files.
- pygrep:  Greps on all local Python files.
- godir:   Go to the directory containing a file.
- goproj:  Go to the root directory of specific code base.
- refresh: Rebuild the tree cache.
- reload:  Reload shell setting from .bashrc.
...
function goproj () {
    ...
    (cd $WORKSRC; find $WORKSRC -maxdepth 1 \( -type d -o -type l \) > projlist)
    echo " Done"
    echo ""
    local lines
    lines=($(grep "$target" $WORKSRC/projlist | sort | uniq))
    if [[ ${#lines[@]} = 0 ]]; then
        echo "Not found"
        return
    fi
