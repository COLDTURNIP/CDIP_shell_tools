    ...
    Darwin)
        function sgrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                find -E . -type f -iregex '.*\.(c|h|cpp|S|java|xml|sh|mk)' -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
            fi
        }
    ...
    *)
        function sgrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                find . -type f -iregex '.*\.\(c\|h\|cpp\|S\|java\|xml\|sh\|mk\)' -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
            fi
        }
        ;;
esac
...
function jgrep()
{
    ...
    if [[ -f $T/filelist_j ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/" $T/filelist_j | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f -name "*\.java" -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function cgrep()
{
    ...
    if [[ -f $T/filelist_c ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/" $T/filelist_c | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f \( -name '*\.c' -o -name '*\.cc' -o -name '*\.cpp' -o -name '*\.h' \) -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function hgrep()
{
    ...
    if [[ -f $T/filelist_c ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*\.h$" $T/filelist_c | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f -name "*\.h*" -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function resgrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath\\(/.*\\)*/res/.*\\.xml$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -n "$@" 2>/dev/null
        )
    else
        for dir in `find . -name res -type d`; do find $dir -type f -name '*\.xml' -print0 | xargs -0 grep -H --color -n "$@"; done; 2>/dev/null
    fi
}
...
function xmlgrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*\\.xml$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -n "$@" 2>/dev/null
        )
    else
        for dir in `find . -name res -type d`; do find $dir -type f -name '*\.xml' -print0 | xargs -0 grep -H --color -n "$@" 2>/dev/null ; done;
    fi
}
...
function pygrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*\\.py$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f -name "*\.py" -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function vgrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*\\.\(\(vala\)\|\(vapi\)\)$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f \( -name '*\.vala' -o -name '*\.vapi' \) -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function gogrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*\\.go$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
        )
    else
        find . -type f -name "*\.go" -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
    fi
}
...
function allgrep()
{
    ...
    if [[ -f $T/filelist ]]; then
        ...
        (
            local rpath=$(getrelative)
            local HERE=$PWD
            grep "^$rpath/.*" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -n "$@" 2>/dev/null
        )
    else
        find . -type f -print0 | xargs -0 grep -H --color -n "$@" 2>/dev/null
    fi
}
        ...
        function mgrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                (
                    local rpath=$(getrelative)
                    local HERE=$PWD
                    grep "^$rpath/.*\\<\\(Makefile\\|Makefile\\..*\\|.*\\.make\\|.*\\.mak\\|.*\\.mk\\)$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
                )
            else
                find -E . -type f -iregex '.*/(Makefile|Makefile\..*|.*\.make|.*\.mak|.*\.mk)' -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
            fi
        }
        ...
        function treegrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                (
                    local rpath=$(getrelative)
                    local HERE=$PWD
                    grep "^$rpath/.*\\.\\(c\\|h\\|cpp\\|S\\|java\\|xml\\)$" $T/filelist | sed -e "s:^$rpath:\\.:" | xargs grep -H --color -C 1 -n "$@" 2>/dev/null
                )
            else
                find -E . -type f -iregex '.*\.(c|h|cpp|S|java|xml)' -print0 | xargs -0 grep -H --color -C 1 -n -i "$@" 2>/dev/null
            fi
        }
    ...
    *)
        function mgrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                find . -regextype posix-egrep -iregex '(.*\/Makefile|.*\/Makefile\..*|.*\.make|.*\.mak|.*\.mk)' -type f -print0 | xargs -0 grep -H --color -C 1 -n "$@" 2>/dev/null
            fi
        }
        ...
        function treegrep()
        {
            ...
            if [[ -f $T/filelist ]]; then
                ...
                find . -regextype posix-egrep -iregex '.*\.(c|h|cpp|S|java|xml)' -type f -print0 | xargs -0 grep -H --color -C 1 -n -i "$@" 2>/dev/null
            fi
        }
